...
const props = defineProps(['replyMessageId', 'replyName'])
...
function messageCommitHandler() {
  formEl.value.validate((isValid: boolean) => {
    if (isValid) {
      ...
      if (!props.replyMessageId && !props.replyName) {
        const messageInfo = {
          name: formValue.name,
          avatar: avatarUrl,
          content: formValue.content
        }
        messageStore.postMessageAction(messageInfo).then(
          () => {
      ...
      } else if (props.replyMessageId && props.replyName) {
        ...
        messageStore.replyMessageAction(messageInfo, props.replyMessageId).then(
          () => {
            emit('postSuccessEmit')
            formEl.value.resetFields()
            ElMessage({
